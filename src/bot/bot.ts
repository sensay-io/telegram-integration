...
export const handleTelegramBot = ({
  bot,
  botUsername,
  replicaUuid,
  overridePlan,
  ...
  bot.on("message:photo", async (ctx) => {
    ...
    try {
      ...
      const messageText = removeMentionIfNeeded(
        caption,
        botUsername,
        needsReply,
      );
...
async function replyToPrivateMessage({
  ...
  try {
    ...
    const { voice, token, usage } =
      await isUserAskingForSnsyTokenOrVoiceRecording(messageText);
...
const publicMessageResponse = async ({
  isTopicMessage,
  parsedChat,
  messageThreadId,
  botUsername,
  ...
  try {
    ...
    const messageTextWithoutMention = removeMentionIfNeeded(
      text,
      botUsername,
      needsReply,
    );
